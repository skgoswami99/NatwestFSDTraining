...
function Recharge() {
  ...
  //   function addbalance () {
  //   fetch(`localhost:8182/auth/addBal/${localStorage.getItem('email')}`, {
  //     method: 'POST',
  //     headers: {
  ...
  const formik = useFormik({
    ...
    validationSchema: yup.object().shape({
      ...
      MobileNumber: yup.string()
        ...
        .required('Number cannot be left blank')
    }),
  });
        ...
        <MDBCol col="12">
          ...
          >
            <MDBCardBody className="p-4 px-5 d-flex flex-column align-items-center mx-auto w-100" >
              <Box sx={{ bgcolor: '#0f2862', px: 2, borderRadius: 1 }} className="fw-bold fs-2 mb-3 text-camelcase text-white">Mobile <ElectricBoltSharp fontSize="" color='warning' /> TopUp
              </Box>
              ...
              <form onSubmit={formik.handleSubmit}>
                ...
                />
                {formik.errors.MobileNumber && formik.touched.MobileNumber ? <span className="text-danger fw-bolder">{formik.errors.MobileNumber} </span> : null}
                <br></br>
                <MDBInput
                  wrapperClass=" w-100"
                ...
                <Box sx={{ display: 'flex', flexDirection: 'row', color: '#0f2862', justifyContent: 'space-evenly', alignItems: 'center' }}>
                  ...
                  <img class="rechargeImg" src={logo} alt='' ></img>
                </Box>
                ...
                <Box sx={style}>
                  {(balance) ?
                    <Typography id="modal-modal-title" variant="h6" component="h2">
                      Recharge Successful !!
                    </Typography> : <h3> Ooops, Go Back to Recharge Your Phone! </h3>}
                  <br></br>
                  {(balance) ? <h3> Wallet Balance Left: {balance} </h3> : null}
                  <br></br>
                  <Button href="/topup" sx={{ bgcolor: '#0f2862', p: 0 }}>
